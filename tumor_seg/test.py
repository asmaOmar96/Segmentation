...
def showPredictsById(case, model, device, start_slice=6):
    """
    ...
    path = f"MICCAI_BraTS_2019_Data_Training/LGG/{case}"
    ...
    pred_labels = np.argmax(p[start_slice], axis=-1)
    ...
    edema_resized = cv2.resize(
    edema[start_slice], (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_NEAREST
    )

    core_resized = cv2.resize(
    core[start_slice], (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_NEAREST
     )

    enhancing_resized = cv2.resize(
    enhancing[start_slice], (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_NEAREST
     )
    ...
    curr_gt = cv2.resize(gt[:, :, start_slice + VOLUME_START_AT], (IMG_SIZE, IMG_SIZE), 
                        interpolation=cv2.INTER_NEAREST)
    axarr[1].imshow(curr_gt, cmap=cmap, interpolation='none', alpha=0.8)
    axarr[1].set_title('Ground truth')
    
    # All classes predicted
    axarr[2].imshow(pred_labels_resized, cmap=cmap, interpolation='none', alpha=0.8)
    axarr[2].set_title('All classes predicted')
    
    # Individual class predictions    
    axarr[3].imshow(brain_bg, cmap="gray")
    axarr[3].imshow(edema_resized, cmap="Blues", interpolation='none', alpha=0.8)
    axarr[3].set_title(f'{SEGMENT_CLASSES[1]} predicted')

    axarr[4].imshow(brain_bg, cmap="gray")
    axarr[4].imshow(core_resized, cmap="Reds", interpolation='none', alpha=0.8)
    axarr[4].set_title(f'{SEGMENT_CLASSES[2]} predicted')
 
    axarr[5].imshow(brain_bg, cmap="gray")
    axarr[5].imshow(enhancing_resized, cmap="Greens", interpolation='none', alpha=0.8)
    axarr[5].set_title(f'{SEGMENT_CLASSES[3]} predicted')    
    plt.savefig('test segmentations.png')
    
    plt.figure()
    plt.imshow(p[start_slice, :, :, 1], cmap='hot')
    plt.colorbar()
...
#print(test_ids[:2])
showPredictsById(case=test_ids[0], model=model, device=device)
